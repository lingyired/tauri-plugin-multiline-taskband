...
3. The window is created with `WS_EX_LAYERED | WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE`:
   ...
   - *Transparent* → mouse clicks pass through to the real taskbar beneath (the plugin is display-only in v1; see [Roadmap](#roadmap)).
...
- [x] Clicking the text region still reaches the taskbar beneath (click-through, `WS_EX_TRANSPARENT`).
...
- **Clickable regions + popup** (mirroring the macOS plugin's click→popup): drop `WS_EX_TRANSPARENT` and handle `WM_LBUTTONUP` to emit a click event + anchor a Tauri popup window. Trade-off: that region would then intercept taskbar clicks; make it opt-in per instance.
