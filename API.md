...
- **Tintable assets**: `tint: true` discards the asset's colours entirely — only the alpha distribution remains, per-pixel, as coverage. Feed it monochrome silhouettes or line art (the fill colour is irrelevant); translucency paints at partial concentration. Coloured or gradient artwork is flattened to a single-colour shape, so keep `tint` off for brand-coloured icons. The demo ships 16 built-in presets (`examples/demo/src/icons/`, embedded in `src/iconPresets.js` — SVG sources plus the `fund01` PNG as a data URL) picked from the popup's dropdown and delivered through the `data` channel — no file path needed.
- **PNG and ICO/BMP in tint mode**: bitmaps don't have the geometric `alpha = 0 outside the shape` separation that SVG paths do — every pixel carries a colour, and the tint renderer only reads the alpha channel. A multi-colour PNG with a transparent background (e.g. a brand mark on a transparent canvas) still renders as the *outer silhouette* in the line colour, with the inner colour details flattened. If the asset's "shape" is the whole rectangle (no transparent surround), `tint: true` will paint the entire icon as a solid line-colour block. Built-in presets that have this risk are tagged `tintRecommended: false`; the demo's preset picker turns Tint off automatically when one of those is chosen, but the user can still toggle it on.
